...
export default class TouchSwitch  {
    ...
    constructor(){
	        this.MAX_ARRAYS[0] = this.TOUCHSWITCH_WIFI_WITH_RELAY;
	        this.MAX_ARRAYS[1] = this.WIFI_RELAY;
	        this.MAX_ARRAYS[2] = this.TOUCHSWITCH_RS485_WITH_RELAY;
	        ...
	        this.MAX_ARRAYS[12] = this.TOUCHSWITCH_WIFI_WITHOUT_RELAY;
	        this.MAX_ARRAYS[13] = this.TOUCHSWITCH_RS485_WITHOUT_RELAY;
    }

    // Scan all wired modules from controller
    scanWiredModules(){
        return new Promise((resolve, reject) => {
            udpScan = new UDP(Commands.REQ_TABLET_MB_COM, Commands.FLAG_REQUEST_RS485_SCAN, "");
            udpScan.sendUdpPacket("", "", true).then(
                dataWiredMods => {
                    getResponse = 1
                    getError = 0
...
//                console.log("error in update : "+ error)
    ...
    // Save TouchSwitch in Controller
    saveTouchSwitchInController(touchSwitch){
        return new Promise((resolve, reject) => {
...
//                 console.log("error 5 " + error)
    ...
    updateTouchSwitchInController(touchSwitchIns) {

        params = new Array()
        params[0] = touchSwitchIns.type_id
        params[1] = touchSwitchIns.type
        params[2] = touchSwitchIns.modeRgb //(color != null) ? 1 : mode

        string_color =  (touchSwitchIns.color != null) ?  tinycolor(touchSwitchIns.color).toHexString().substr(1,6) : "ff0096" //tinycolor(this.state.color).toHexString().substr(1,6)
//        console.log("selected color:    " + string_color +"--" + color)
        ...
        params[6] = touchSwitchIns.speed
        params[7] = touchSwitchIns.start
        params[8] = touchSwitchIns.end
        params[9] = touchSwitchIns.applyToAll

        udpRgb = new UDP(Commands.REQ_TABLET_MB_COM, Commands.FLAG_RGB_SWITCH_SET, params)
        return new Promise((resolve, reject) => {
...
//    		console.log("error in update colorChange: "+ error)
    ...
    updateTouchSwitchInDB(touchSwitch){
        params = new Array();
        params[0] = touchSwitch.title;
        params[1] = touchSwitch.flag;

        return new Promise((resolve, reject) => {
            ZagrosDB.buildQuery(Vars.queryUpdate, "TouchSwitch", "title, flag", "id = " + touchSwitch.id , params, "", "", 0, 0).then(
               data => {
                    resolve(true)
               }
            )
    ...
    // Update flag in DB
    getAllTouchSwitchesFromController(touchesFromDB){

        return new Promise((resolve, reject) => {
            params = new Array()
            params[0] = 0
           
            ZagrosDB.buildQuery(Vars.queryUpdate, "TouchSwitch", "flag", "", params, "", "", 0).then(
               data => {
               

            udpGetT = new UDP(Commands.REQ_TABLET_MB_COM, Commands.FLAG_LISTING_MODULES, "");
...
//              console.log("error in update : "+ error)
    ...
    getTouchSwitch(touchSwitchType, touchSwitchTypeId){
          params = new Array()
            params[0] = touchSwitchTypeId
            params[1] = touchSwitchType
//            console.log("params to send: " + params[0]+"---"+params[1])
        return new Promise((resolve, reject) => {
	           udpGetTo = new UDP(Commands.REQ_TABLET_MB_COM, Commands.FLAG_RGB_SWITCH_GET, params);
...
//	                               dataListUdp[9] + "--"+dataListUdp[10] + "--"  )
    ...
    updateTouchSwitchesFromController(dataFromController, touchSwitchesFromDB){
        return new Promise((resolve, reject) => {
            try{
                ...
                for (b = 0; b < len; b++) {
                    ...
                    touchSwitchesFromDB[b].flag = wifiBytes[0] & 1;
//                    alert("i: " + b + "--" + touchSwitchesFromDB[b].flag + "--" + (wifiBytes[0] & 1))
//                    if(b == len-1){

//                    }
                }

                resolve(touchSwitchesFromDB);


            }
            catch(error){
                alert(i18n.t("touchSwitch:errorGetAllTouchSwitches"));
                reject(i18n.t("touchSwitch:errorGetAllTouchSwitches"))
            }
        })
    }
    ...
    makeTable(){
        try{
            sqlMakeTable = "CREATE TABLE IF NOT EXISTS  [TouchSwitch] ("
            ...
            alert(i18n.t("touchSwitch:errorCreateTable"));
            //            return 0;
        }
    }

    // Get next id that can be taken for new TouchSwitch
    getNextId(type){
        return new Promise((resolve, reject) => {
            ZagrosDB.buildQuery(Vars.querySelect, "TouchSwitch", "MIN(id) AS id, type_id", "(flag = 0 AND type = " + type + ")", "", "", "",1,0).then(
                data2 => {
            ...
            .catch(
                error => {
                    alert(i18n.t("touchSwitch:errorGetNextId"));
                    reject(i18n.t("touchSwitch:errorGetNextId"))
                }
            );
        }) // End Promise
    }

    createTouchSwitches(t){
       return new Promise((resolve, reject) => {
            try{
                 ZagrosDB.buildQuery(Vars.queryDelete, "TouchSwitch", "", "", "", "", "").then(
                 data1 => {

                    this.createAllTouchSwitches(t).then(
                        res => {
                            resolve(res)
                        }
                    )
                    .catch(error => {
                        alert(i18n.t("touchSwitch:createAllTouchSwitches"))
                        reject(i18n.t("touchSwitch:createAllTouchSwitches"))
                    });
    ...
    createAllTouchSwitches(t){
        return new Promise((resolve, reject) => {

            try{
                paramsw = new Array();

                sqlInsWifi = "INSERT INTO TouchSwitch(title, type, type_id) VALUES ";

                type_id = 1
                // Insert all wifi touchSwitch with relay
                for(c = 1; c <= this.TOUCHSWITCH_WIFI_WITH_RELAY; c++){
                    if(c != 1){
                        sqlInsWifi += ",";
                    }
                    sqlInsWifi += "(?," + this.TOUCHSWITCH_WIFI_WITH_RELAY_TYPE + ", " + type_id + ")";
                    paramsw[c-1] = t('touchSwitch:touchSwitchWifiWithRelay') + " " + type_id;
                    type_id++
                }

                // Insert all wifi relay touch switches
                max = this.TOUCHSWITCH_WIFI_WITH_RELAY + this.TOUCHSWITCH_RS485_WITH_RELAY
                from = this.TOUCHSWITCH_WIFI_WITH_RELAY + 1
                type_id = 1
                for(r = from; r <= max; r++){
                    sqlInsWifi += ",(?," + this.TOUCHSWITCH_RS485_WITH_RELAY_TYPE + ", " + type_id + ")";
                    paramsw[r-1] = t('touchSwitch:touchSwitchRs485WithRelay') + " " + type_id;
                    type_id++
                }

                // Insert all wifi without relay touch switches
                max = max + this.TOUCHSWITCH_WIFI_WITHOUT_RELAY
                from = from + this.TOUCHSWITCH_RS485_WITH_RELAY
                type_id = 1
                for(d = from; d <= max; d++){
                    sqlInsWifi += ",(?," + this.TOUCHSWITCH_WIFI_WITHOUT_RELAY_TYPE + ", " + type_id + ")";
                    paramsw[d-1] = t('touchSwitch:touchSwitchWifiWithoutRelay') + " " + type_id;
                    type_id++
                }

                // Insert all wifi without relay touch switches
                max = max + this.TOUCHSWITCH_RS485_WITHOUT_RELAY
                from = from + this.TOUCHSWITCH_WIFI_WITHOUT_RELAY
                type_id = 1
                for(e = from; e <= max; e++){
                    sqlInsWifi += ",(?," + this.TOUCHSWITCH_RS485_WITHOUT_RELAY_TYPE + ", " + type_id + " )";
                    paramsw[e-1] = t('touchSwitch:touchSwitchRs485WithoutRelay') + " " + type_id;
                    type_id++
                }

                ZagrosDB.executeSQL(sqlInsWifi, paramsw, 0).then(
                    data => {
...
//                            console.log("wifiiiiii"+data2[0].count)
    ...
    deleteTouchSwitch(touchSwitchId, type, type_id, retry){
        if(!retry){ retry = 5; }
        let getResponse = 0
        let getError = 0
        ...
        return new Promise((resolve, reject) => {
            ...
            params1[0] = type_id;
            params1[1] = type;
            udpDelT = new UDP(Commands.REQ_TABLET_MB_COM, Commands.FLAG_DELETE_WIRELESS, params1)
            udpDelT.sendUdpPacket("", "", true).then(
                data => {
                   getResponse = 1
                   getError = 0
...
//                            alert(params1[0] + "-" + data[0] + "-" + data[1] + "-" + data[2] +"-" +data[3]+"-"+ data[4]+"-"+ data[5])
                        ...
                        // Delete selected touchSwitch. set status to 0
                        ZagrosDB.buildQuery(Vars.queryUpdate, "TouchSwitch", "flag", "id="+touchSwitchId, params, "", "", 0, 0).then(
                           data => {
                                resolve(true);
                           }
                        )
                        .catch(
                           error => {
                                reject(i18n.t("touchSwitch:errorDeleteTouchSwitch"));
                           }
                        );
                   }
                }
            ...
            setTimeout(() => {
                if(getResponse == 0 && getError == 0){
                  if(retry > 0){
                    this.deleteTouchSwitch(touchSwitchId, retry-1)
                  }
                  else {
                    reject(false)
                  }
